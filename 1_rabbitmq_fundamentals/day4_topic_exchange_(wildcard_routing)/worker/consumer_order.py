...
def main():
    ...
    while not connection:
        try:
            print('[ORDER Service] is connecting to RabbitMQ...')
            connection = pika.BlockingConnection(params)
            print('[ORDER Service] connected to RabbitMQ successfully.')
        except pika.exceptions.AMQPConnectionError:
            print('[ORDER Service] failed to connect to RabbitMQ. Retrying in 5 seconds...')
            time.sleep(5)
    ...
    print('[ORDER Worker] Listening for order events...')

    def callback(ch, method, properties, body):
        data = json.loads(body)
        print(f"[ORDER Worker] Received: {method.routing_key} -> {data}")
